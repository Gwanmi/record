...
- 웹 서버-WAS 구성
  ...
  - Web Server와 WAS의 차이
    ...
    - 웹 컨테이너 (Container): 동적인 데이터들을 처리하여 정적인 페이지로 생성해주는 소프트웨어 모듈이다. Apache는 CGI라는 개념을 지원하는데 PHP, Perl, Python등의 언어는 Apache를 통해 CGI를 적용시키는 것이 가능하지만, JAVA는 안된다. 즉, Java는 따로 CGI 와 같은 기능을 위해 컨테이너 라는 것이 필요한데 그것이 서블릿이다.
